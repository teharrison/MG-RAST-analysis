...
# help
if [ $HELP -eq 1 ]; then
    echo "Usage: init-kbnb.sh [-h -n] -t USER_TOKEN -s SHOCK_SERVER -p PORT -b BUILD_MODE(none|ipython|all)"
    exit
fi
...
sleep 1
/kb/deployment/services/analysis_book/start_service -a 'globus' -t "$USER_TOKEN" -s "$SHOCK_SERVER" -i $PORT
